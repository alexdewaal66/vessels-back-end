...
public abstract class GenericConversionController<S, T extends GenericEntity<T>> {
    ...
    public GenericConversionController(GenericServiceImpl<T> service) {
...
//        System.out.println("GenericController » getByIds \n\t ids=" + ids);
    ...
    public ResponseEntity<Object> findItemsByExample(@RequestBody Match<S> remoteMatch) {
        Match<T> match = new Match<T>();
        match.setMode(remoteMatch.getMode());
        match.setProbe(convert(remoteMatch.getProbe()));
        List<T> items = service.findAllByExample(match);
        return ResponseEntity.ok().body(items);
    }

    @PostMapping(value = "/findone")
    public ResponseEntity<Object> findItemByExample(@RequestBody Match<S> remoteMatch) {
        Match<T> match = new Match<T>();
        match.setMode(remoteMatch.getMode());
        match.setProbe(convert(remoteMatch.getProbe()));
